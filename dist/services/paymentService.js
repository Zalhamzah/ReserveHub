...
class PaymentService {
    ...
    stripe;
    constructor() {
        this.stripe = new stripe_1.default(config_1.config.payment.stripe.secretKey, {
     ...
     */
    async createBookingPaymentIntent(bookingId, amount, currency = 'usd', description) {
        try {
            // Get booking details
            ...
            if (!booking) {
                throw new errorHandler_1.ValidationError('Booking not found');
            }
            ...
            logger_1.logger.info(`Payment intent created: ${paymentIntent.id} for booking ${booking.bookingNumber}`);
            return {
                id: paymentIntent.id,
                amount,
                currency,
    ...
    async confirmPaymentIntent(paymentIntentId) {
        try {
            ...
            logger_1.logger.info(`Payment intent confirmed: ${paymentIntentId}`);
            return {
                id: paymentIntent.id,
     ...
     */
    async createStripeCustomer(customerId, email, name, phone) {
        try {
            ...
            logger_1.logger.info(`Stripe customer created: ${stripeCustomer.id} for customer ${customerId}`);
    ...
    async savePaymentMethod(customerId, paymentMethodId) {
        try {
            ...
            if (!customer) {
                throw new errorHandler_1.ValidationError('Customer not found');
            }
            let stripeCustomerId = customer.stripeCustomerId;
            // Create Stripe customer if doesn't exist
            if (!stripeCustomerId) {
                stripeCustomerId = await this.createStripeCustomer(customerId, customer.email || '', `${customer.firstName} ${customer.lastName}`);
            }
            ...
            logger_1.logger.info(`Payment method saved: ${paymentMethodId} for customer ${customerId}`);
            return result;
        }
        catch (error) {
            logger_1.logger.error('Error saving payment method:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async processBookingPayment(bookingId, amount, paymentMethodId, currency = 'usd') {
        try {
            // Get booking details
            const booking = await database_1.prisma.booking.findUnique({
                where: { id: bookingId },
            ...
            logger_1.logger.info(`Payment processed: ${paymentIntent.id} for booking ${booking.bookingNumber}`);
            return {
                id: paymentIntent.id,
                amount,
                currency,
            ...
            logger_1.logger.error('Error processing booking payment:', error);
            throw error;
        }
    }
    /**
    ...
    async refundPayment(paymentId, amount, reason) {
        try {
            ...
            if (!payment) {
                throw new errorHandler_1.ValidationError('Payment not found');
            }
            if (payment.status !== 'COMPLETED') {
                throw new errorHandler_1.ConflictError('Can only refund completed payments');
            }
            ...
            logger_1.logger.info(`Payment refunded: ${paymentId} - ${refund.amount / 100} ${payment.currency}`);
            return {
                id: refund.id,
    ...
    async handleWebhook(event) {
        try {
            logger_1.logger.info(`Handling Stripe webhook: ${event.type}`);
            switch (event.type) {
    ...
    async getBookingPayments(bookingId) {
        try {
            ...
            logger_1.logger.error('Error retrieving booking payments:', error);
            throw error;
        }
    }
    /**
     ...
     */
    calculateBookingTotal(baseAmount, taxRate = 0.0875) {
    ...
    async handleChargeDispute(dispute) {
        try {
            ...
            logger_1.logger.warn(`Charge dispute created: ${dispute.id} for charge ${dispute.charge}`);
