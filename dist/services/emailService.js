...
class EmailService {
    ...
    constructor() {
        this.transporter = nodemailer_1.default.createTransporter({
            host: config_1.config.notifications.email.host,
            port: config_1.config.notifications.email.port,
            secure: config_1.config.notifications.email.secure,
            auth: {
                user: config_1.config.notifications.email.user,
                pass: config_1.config.notifications.email.password
            }
        });
    }
    async sendEmail(emailData) {
        try {
            const mailOptions = {
                from: config_1.config.notifications.email.from,
                to: emailData.to,
            ...
            logger_1.logger.info(`Email sent successfully to ${emailData.to}`, { messageId: result.messageId });
            return true;
        }
        catch (error) {
    ...
    async sendBookingReminder(data) {
        try {
            ...
            logger_1.logger.error('Error sending booking reminder:', error);
            return false;
        }
    }
    async sendBookingCancellation(data) {
        try {
            ...
            logger_1.logger.error('Error sending booking cancellation:', error);
            return false;
        }
    }
    ...
    getBookingCancellationTemplate(data) {
    ...
    getWaitlistNotificationTemplate(data) {
